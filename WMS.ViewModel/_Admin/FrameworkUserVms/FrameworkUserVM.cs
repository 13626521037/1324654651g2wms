...
namespace WMS.ViewModel._Admin.FrameworkUserVMs
{
    public partial class FrameworkUserVM : BaseCRUDVM<FrameworkUser>
    {
        ...
        {
            SetInclude(x => x.BaseUserWhRelation_User);
        }

        public void SetWhDetail()
        {
            if (Entity.BaseUserWhRelation_User != null && Entity.BaseUserWhRelation_User.Count > 0)
            {
                foreach (var item in Entity.BaseUserWhRelation_User)
                {
                    item.Wh = DC.Set<BaseWareHouse>().Find(item.WhId);
                }
            }
        }
