...
namespace WMS.SalesManagement.Controllers
{
    ...
    [ActionDescription("_Model.SalesShip")]
    public partial class SalesShipController : BaseController
    {
        #region Create
        [HttpPost]
        [ActionDescription("Sys.Create")]
        public async Task<ActionResult> Create(SalesShipVM vm)
        {
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    vm.DoReInit();
                    return PartialView("../SalesShip/Create", vm);
                }
                else
                {
        ...
        public async Task<ActionResult> Edit(SalesShipVM vm)
        {
            if (!ModelState.IsValid)
            {
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    vm.DoReInit();
                    return PartialView("../SalesShip/Edit", vm);
                }
                else
                {
        ...
        public ActionResult DoBatchEdit(SalesShipBatchVM vm, IFormCollection nouse)
        {
        ...
        {
            var vm = Wtm.CreateVM<SalesShipBatchVM>();
        ...
        public ActionResult Import(SalesShipImportVM vm, IFormCollection nouse)
        {
        ...
        public ActionResult Select_GetSalesShipByBaseOrganization(List<string> id)
        {
            var rv = DC.Set<SalesShip>().CheckIDs(id, x => x.OrganizationId).GetSelectListItems(Wtm,x=>x.SourceSystemId);
            return JsonMore(rv);
        }
        ...
        public ActionResult Select_GetSalesShipByBaseCustomer(List<string> id)
        {
            var rv = DC.Set<SalesShip>().CheckIDs(id, x => x.CustomerId).GetSelectListItems(Wtm,x=>x.SourceSystemId);
            return JsonMore(rv);
        }
