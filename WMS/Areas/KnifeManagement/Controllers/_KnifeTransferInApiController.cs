...
namespace WMS.KnifeManagement.Controllers
{
    ...
    [ActionDescription("_Model.KnifeTransferIn")]
    public partial class KnifeTransferInController : BaseController
    {
        #region Create
        [HttpPost]
        [ActionDescription("Sys.Create")]
        public async Task<ActionResult> Create(KnifeTransferInVM vm)
        {
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    vm.DoReInit();
                    return PartialView("../KnifeTransferIn/Create", vm);
                }
                else
                {
        ...
        public async Task<ActionResult> Edit(KnifeTransferInVM vm)
        {
            if (!ModelState.IsValid)
            {
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    vm.DoReInit();
                    return PartialView("../KnifeTransferIn/Edit", vm);
                }
                else
                {
        ...
        public ActionResult DoBatchEdit(KnifeTransferInBatchVM vm, IFormCollection nouse)
        {
        ...
        {
            var vm = Wtm.CreateVM<KnifeTransferInBatchVM>();
        ...
        public ActionResult Import(KnifeTransferInImportVM vm, IFormCollection nouse)
        {
        ...
        public ActionResult Select_GetKnifeTransferInByFrameworkUser(List<string> id)
        {
            var rv = DC.Set<KnifeTransferIn>().CheckIDs(id, x => x.HandledById).GetSelectListItems(Wtm,x=>x.DocNo);
            return JsonMore(rv);
        }
        ...
        /*public ActionResult Select_GetKnifeTransferInByBaseWhArea(List<string> id)
        {
            var rv = DC.Set<KnifeTransferIn>().CheckIDs(id, x => x.WhLocationId).GetSelectListItems(Wtm,x=>x.DocNo);
            return JsonMore(rv);
        }*/
