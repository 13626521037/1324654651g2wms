...
{
    public partial class PurchaseOutsourcingIssueApiVM : BaseCRUDVM<PurchaseOutsourcingIssue>
    {
        /// <summary>
        /// 自动拆码结果
        /// </summary>
        ...
        public PurchaseOutsourcingIssueApiVM()
        {
        ...
        /// <returns></returns>
        public List<PurchaseOutsourcingIssueReturn> GetList(int type = 0)
        {
            ...
            List<PurchaseOutsourcingIssueReturn> list = new List<PurchaseOutsourcingIssueReturn>();
            Expression<Func<PurchaseOutsourcingIssue, bool>> queryable;
            if (type == 0)  // 待下架（未全部下架）。需匹配登录用户的存储地点
            {
                queryable = x => (x.Status == Model.PurchaseOutsourcingIssueStatusEnum.InWh
                    || x.Status == PurchaseOutsourcingIssueStatusEnum.PartOff)
                    && x.PurchaseOutsourcingIssueLine_OutsourcingIssue.Any(y => y.WareHouseId == whid);
            }
            else
            {
                queryable = x => x.Status == PurchaseOutsourcingIssueStatusEnum.AllOff
                    && x.PurchaseOutsourcingIssueLine_OutsourcingIssue.Any(y => y.WareHouseId == whid);
            }
            list = DC.Set<PurchaseOutsourcingIssue>()
               .Where(queryable)
               .AsNoTracking()
               .Select(x => new PurchaseOutsourcingIssueReturn
               {
                   ID = x.ID,
                   DocNo = x.DocNo,
                   DocType = x.DocType,
                   ...
                   SumQty = (int)(x.PurchaseOutsourcingIssueLine_OutsourcingIssue.Sum(y => y.Qty) ?? 0),
        ...
        public void Create(PurchaseOutsourcingIssue entity)
        {
            MSD.Clear();    // 看下是否需要判断
            var existEntity = DC.Set<PurchaseOutsourcingIssue>().Where(x => x.DocNo == entity.DocNo).FirstOrDefault();
            if (existEntity != null)
            {
            ...
            foreach (var line in entity.PurchaseOutsourcingIssueLine_OutsourcingIssue)
            {
                line.Status = Model.PurchaseOutsourcingIssueLineStatusEnum.InWh;
            ...
            // 将单据保存到本系统中
            Entity = entity;
            Entity.Status = Model.PurchaseOutsourcingIssueStatusEnum.InWh;
            DoAdd();
        }
        ...
        public PurchaseOutsourcingIssueReturn GetDoc(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            PurchaseOutsourcingIssue doc = DC.Set<PurchaseOutsourcingIssue>()
                ...
                .Include(x => x.PurchaseOutsourcingIssueLine_OutsourcingIssue)
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                if (doc.PurchaseOutsourcingIssueLine_OutsourcingIssue.Any(y => y.WareHouseId != whid))
                {
                    MSD.AddModelError("", $"单据{docNo}的发料存储地点与当前登录用户的存储地点{loginWh.Code}不匹配");
                    return null;
                }
                // 判断单据状态：只有在库和部分下架的才允许操作
                if (doc.Status != Model.PurchaseOutsourcingIssueStatusEnum.InWh && doc.Status != Model.PurchaseOutsourcingIssueStatusEnum.PartOff)
                {
                    MSD.AddModelError("", $"当前单据状态“{doc.Status.GetEnumDisplayName()}”不允许进行下架操作");
                    return null;
                }
                return new PurchaseOutsourcingIssueReturn(doc);
            }
            // 本系统不存在时，尝试从ERP获取
            ...
            U9Return<PurchaseOutsourcingIssue> u9Return = apiHelper.GetPurchaseOutsourcingIssue(docNo);
            if (u9Return.Success)
            {
                // 引用类型外部系统ID转换
                ...
                r = this.SSIdAttrToId(u9Return.Entity, u9Return.Entity.Supplier, "Supplier");   // 调出组织
                if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                foreach (var line in u9Return.Entity.PurchaseOutsourcingIssueLine_OutsourcingIssue)
                {
                    r = this.SSIdAttrToId(line, line.ItemMaster, "ItemMaster");   // 转换物料
                    if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                    r = this.SSIdAttrToId(line, line.WareHouse, "WareHouse");   // 转换存储地点
                ...
                if (u9Return.Entity.PurchaseOutsourcingIssueLine_OutsourcingIssue.Any(y => y.WareHouseId != whid))
                {
                    MSD.AddModelError("", $"单据{docNo}的发料存储地点与当前登录用户的存储地点{loginWh.Code}不匹配");
                    return null;
                }
                foreach (var line in u9Return.Entity.PurchaseOutsourcingIssueLine_OutsourcingIssue)
                {
                    line.Status = PurchaseOutsourcingIssueLineStatusEnum.InWh;
                    line.ToBeOffQty = line.Qty;
                    line.ToBeShipQty = 0;
                    line.OffQty = 0;
                    line.ShippedQty = 0;
                ...
                Entity.Status = PurchaseOutsourcingIssueStatusEnum.InWh;
                DoAdd();
                if (!MSD.IsValid)
                {
                    return null;
                }
                doc = DC.Set<PurchaseOutsourcingIssue>()
                    .Where(x => x.DocNo == docNo)
                    .Include(x => x.Organization)
                    .Include(x => x.Supplier)
                    .Include(x => x.PurchaseOutsourcingIssueLine_OutsourcingIssue)
        ...
        /// <returns></returns>
        public PurchaseOutsourcingIssueReturn GetDocForApprove(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                if (doc.Status != PurchaseOutsourcingIssueStatusEnum.AllOff)
                {
                    MSD.AddModelError("", $"单据{docNo}的状态为{doc.Status.GetEnumDisplayName()}，无法进行审核");
                    return null;
                }
                PurchaseOutsourcingIssueReturn result = new PurchaseOutsourcingIssueReturn(doc);
        ...
        public void GetOffDetails(PurchaseOutsourcingIssueReturn doc)
        {
            var invRelations = DC.Set<BaseDocInventoryRelation>().AsNoTracking().Include(x => x.Inventory.WhLocation.WhArea.WareHouse)
                .Where(x => x.BusinessId == doc.ID && x.DocType == DocTypeEnum.OutsourcingIssue).ToList();
            ...
            {
                line.SubLines = new List<PurchaseOutsourcingIssueReturnSubLine>();
                invRelations.FindAll(x => x.BusinessLineId == line.ID).ForEach(x =>
                {
                    ...
                    //    return;
                    //}
                    line.SubLines.Add(new PurchaseOutsourcingIssueReturnSubLine
                    {
        ...
        /// </summary>
        /// <param name="doc"></param>
        public void GetSuggestLocs(PurchaseOutsourcingIssueReturn doc)
        {
            foreach (var line in doc.Lines)
            {
                line.SuggestLocs = new List<PurchaseOutsourcingIssueReturnLineSuggestLoc>();
                List<BaseInventory> invs = this.GetItemInventory(line.ItemID, line.WareHouseID);
                if (invs.Count > 0)
                {
                    // invs = invs.OrderBy(x => x.CreateTime).ToList(); // 默认升序
                    foreach (var inv in invs)
                    {
                        line.SuggestLocs.Add(new PurchaseOutsourcingIssueReturnLineSuggestLoc
                        {
        ...
        {
            var entity = DC.Set<PurchaseOutsourcingIssue>().Where(x => x.SourceSystemId == sourceSystemId).AsNoTracking().FirstOrDefault();
            if (entity == null)
            {
                return false;
            }
        ...
        /// </summary>
        /// <param name="doc"></param>
        public void SaveOffDetails(PurchaseOutsourcingIssueSaveOffPara data, bool autoSplit)
        {
            ...
            using (var trans = DC.BeginTransaction())
            {
                try
                {
                    PurchaseOutsourcingIssue doc = DC.Set<PurchaseOutsourcingIssue>()             //.AsNoTracking()
                        .Include(x => x.PurchaseOutsourcingIssueLine_OutsourcingIssue)
                    ...
                    if (doc.PurchaseOutsourcingIssueLine_OutsourcingIssue[0].WareHouseId != whid)
                    {
                        MSD.AddModelError("", "单据存储地点不匹配，可尝试重新选择登录存储地点");
                        return;
                    }
                    if (doc.Status != PurchaseOutsourcingIssueStatusEnum.InWh && doc.Status != PurchaseOutsourcingIssueStatusEnum.PartOff)
                    {
                    ...
                    foreach (var dataLine in data.Lines)
                    {
                        var docline = doc.PurchaseOutsourcingIssueLine_OutsourcingIssue.Find(x => x.ID == dataLine.ID);
                        if (docline == null)
                        {
                            MSD.AddModelError("", $"行数据“{dataLine.ID}”不存在");
                            return;
                        }
                        if (docline.Status != PurchaseOutsourcingIssueLineStatusEnum.InWh && docline.Status != PurchaseOutsourcingIssueLineStatusEnum.PartOff)
                        {
                        ...
                        if (docline.ToBeOffQty == 0)
                        {
                            docline.Status = PurchaseOutsourcingIssueLineStatusEnum.AllOff;
                        }
                        else if (docline.OffQty == 0)
                        {
                            docline.Status = PurchaseOutsourcingIssueLineStatusEnum.InWh;
                        }
                        else
                        {
                            docline.Status = PurchaseOutsourcingIssueLineStatusEnum.PartOff;
                        }
                    }
                    ...
                    // 设置单据状态
                    if (doc.PurchaseOutsourcingIssueLine_OutsourcingIssue.All(x => x.Status == PurchaseOutsourcingIssueLineStatusEnum.AllOff))   // 全部行已下架
                    {
                        doc.Status = PurchaseOutsourcingIssueStatusEnum.AllOff;
                    }
                    else if (doc.PurchaseOutsourcingIssueLine_OutsourcingIssue.All(x => x.Status == PurchaseOutsourcingIssueLineStatusEnum.InWh))  // 全部行未下架（下架动作不会出现这种情况）
                    {
                        doc.Status = PurchaseOutsourcingIssueStatusEnum.InWh;
                    }
                    else
                    {
                        doc.Status = PurchaseOutsourcingIssueStatusEnum.PartOff;
                    }
                    DC.SaveChanges();
                    ...
                    {
                        // 创建库存流水
                        if (!this.CreateInvLog(OperationTypeEnum.OutsourcingIssueOff, doc.DocNo, sourceInvIds[i], newInventoryList[i].ID, -newInventoryList[i].Qty, newInventoryList[i].Qty))
                        {
                            return;
                        }
                    }
                    ...
                    {
                        // 创建单据库存关联关系
                        if (!this.CreateInvRelation(DocTypeEnum.OutsourcingIssue, item.Item2, doc.ID, item.Item1, item.Item3))
                        {
                            return;
                        }
                    }
        ...
        public void CancelOff(Guid? docId)
        {
            ...
            try
            {
                using (var trans = DC.BeginTransaction())
                {
                    var doc = DC.Set<PurchaseOutsourcingIssue>().Include(x => x.PurchaseOutsourcingIssueLine_OutsourcingIssue).Where(x => x.ID == docId).FirstOrDefault();
                    if (doc == null)
                    {
                        MSD.AddModelError("", "单据不存在");
                        return;
                    }
                    if (doc.Status == PurchaseOutsourcingIssueStatusEnum.InWh)
                    {
                        MSD.AddModelError("", "单据尚未进行下架业务，无法进行取消下架操作");
                        return;
                    }
                    if (doc.Status == PurchaseOutsourcingIssueStatusEnum.PartShipped || doc.Status == PurchaseOutsourcingIssueStatusEnum.AllShipped)
                    {
                        MSD.AddModelError("", "单据已出货，无法进行取消下架操作");
                        return;
                    }
                    // 取消下架
                    foreach (var line in doc.PurchaseOutsourcingIssueLine_OutsourcingIssue)
                    {
                        if (line.Status == PurchaseOutsourcingIssueLineStatusEnum.InWh)
                        {
                            continue;
                        }
                        if (line.OffQty <= 0)
                        ...
                        else if (line.Status == PurchaseOutsourcingIssueLineStatusEnum.PartShipped || line.Status == PurchaseOutsourcingIssueLineStatusEnum.AllShipped) // 头部已过滤，不会出现这种情况
                        {
                    ...
                    // 设置单据状态
                    if (doc.PurchaseOutsourcingIssueLine_OutsourcingIssue.All(x => x.Status == PurchaseOutsourcingIssueLineStatusEnum.AllOff))   // 全部行已下架（取消下架动作不会出现这种情况）
                    {
                        doc.Status = PurchaseOutsourcingIssueStatusEnum.AllOff;
                    }
                    else if (doc.PurchaseOutsourcingIssueLine_OutsourcingIssue.All(x => x.Status == PurchaseOutsourcingIssueLineStatusEnum.InWh))  // 全部行未下架
                    {
                        doc.Status = PurchaseOutsourcingIssueStatusEnum.InWh;
                    }
                    else
                    ...
                    foreach (var relation in invRelations)
                    {
                        ...
                        // 创建库存流水
                        if (!this.CreateInvLog(OperationTypeEnum.OutsourcingIssueOffCancel, doc.DocNo, relation.InventoryId, originalInv.ID, -inventory.Qty, inventory.Qty))
                        {
                            return;
                        }
                        // 待发库位的库存信息清零
        ...
        public void Approve(Guid? docId)
        {
            ...
            using (var trans = DC.BeginTransaction())
            {
                var doc = DC.Set<PurchaseOutsourcingIssue>().Include(x => x.PurchaseOutsourcingIssueLine_OutsourcingIssue).ThenInclude(x => x.ItemMaster).Include(x => x.Organization).Where(x => x.ID == docId).FirstOrDefault();
                if (doc == null)
                {
                    MSD.AddModelError("", "单据不存在");
                    return;
                ...
                foreach (var line in doc.PurchaseOutsourcingIssueLine_OutsourcingIssue)
                {
                    line.Status = PurchaseOutsourcingIssueLineStatusEnum.AllShipped;
                    ...
                    foreach (var relation in matchRelations)
                    {
                        ...
                        //}
                        // 创建库存流水
                        this.CreateInvLog(OperationTypeEnum.OutsourcingIssueApprove, doc.DocNo, inventory.ID, null, -relation.Qty, null);
                ...
                doc.Status = PurchaseOutsourcingIssueStatusEnum.AllShipped;
