...
namespace WMS.ViewModel._Admin.FrameworkUserVMs
{
    public partial class FrameworkUserListVM : BasePagedListVM<FrameworkUser_View, FrameworkUserSearcher>
    {
        ...
        protected override List<GridAction> InitGridAction()
        {
            return new List<GridAction>
            {
                ...
                this.MakeAction("FrameworkUser","Password",@Localizer["Login.ChangePassword"].Value,@Localizer["Login.ChangePassword"].Value,GridActionParameterTypesEnum.SingleIdWithNull,"_Admin",800).SetShowInRow(true).SetHideOnToolBar(true).SetIconCls("fa fa-unlock-alt").SetButtonClass("layui-btn-warm").SetBindVisiableColName("Super"),
            };
        }
        ...
        protected override IEnumerable<IGridColumn<FrameworkUser_View>> InitGridHeader()
        {
            return new List<GridColumn<FrameworkUser_View>>{
                ...
                this.MakeGridHeader(x => x.FrameworkUser_ITCode, width: 120).SetTitle(@Localizer["Sys.Account"].Value),
                this.MakeGridHeader(x => x.FrameworkUser_Name, width: 150).SetTitle(@Localizer["_Admin.Name"].Value),
                this.MakeGridHeader(x => x.WarehouseName, width: 200),
                this.MakeGridHeader(x => x.FrameworkUser_Gender, width: 80).SetTitle(@Localizer["_Admin.Gender"].Value),
                this.MakeGridHeader(x => x.FrameworkUser_CellPhone, width: 120).SetTitle(@Localizer["_Admin.CellPhone"].Value),
                ...
                this.MakeGridHeader(x => x.FrameworkUser_IsValid, width: 80).SetTitle(@Localizer["_Admin.IsValid"].Value),
                //this.MakeGridHeader(x => x.FrameworkUser_Photo).SetTitle(@Localizer["_Admin.Photo"].Value).SetFormat(FrameworkUser_PhotoFormat),
                this.MakeGridHeader(x => "Super").SetHide().SetFormat((a, b) =>
                {
                    // 只有超级管理员才能修改拥有超级管理员权限的用户的密码
                    if (a.FrameworkUser_Role.IndexOf("超级管理员") != -1 && !LoginUserInfo.FunctionPrivileges.Any(y => y.RoleCode == "001"))
                    {
                        return "false";
                    }
                    return "true";
        ...
        public override IOrderedQueryable<FrameworkUser_View> GetSearchQuery()
        {
            var query = DC.Set<FrameworkUser>()
                ...
                .Select(x => new FrameworkUser_View
                {
                    ...
                    WarehouseName = DC.Set<BaseWareHouse>().Where(y => y.ID == x.WarehouseId).Select(y0=>y0.Name).FirstOrDefault(),
                })
                .OrderBy(x => x.ID);
            return query;
        }
