...
{
    public partial class SalesRMAApiListVM : BasePagedListVM<SalesRMAApi_View, SalesRMAApiSearcher>
    {

        protected override IEnumerable<IGridColumn<SalesRMAApi_View>> InitGridHeader()
        {
            return new List<GridColumn<SalesRMAApi_View>>{
                this.MakeGridHeader(x => x.CreatePerson),
                this.MakeGridHeader(x => x.Name_view),
                this.MakeGridHeader(x => x.BusinessDate),
                this.MakeGridHeader(x => x.ApproveDate),
                this.MakeGridHeader(x => x.DocNo),
                this.MakeGridHeader(x => x.DocType),
                this.MakeGridHeader(x => x.Operators),
        ...
        public override IOrderedQueryable<SalesRMAApi_View> GetSearchQuery()
        {
            var query = DC.Set<SalesRMA>()
                .Select(x => new SalesRMAApi_View
                {
				    ID = x.ID,
    ...
    public class SalesRMAApi_View : SalesRMA{
        [Display(Name = "名称")]
