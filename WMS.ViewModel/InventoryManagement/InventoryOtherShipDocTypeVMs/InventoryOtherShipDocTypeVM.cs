...
{
    public partial class InventoryOtherShipDocTypeVM : BaseCRUDVM<InventoryOtherShipDocType>
    {
        /// <summary>
        /// 同步更新数量
        /// </summary>
        ...
        public List<string> InventoryManagementInventoryOtherShipDocTypeFTempSelected { get; set; }

        public InventoryOtherShipDocTypeVM()
        {
        ...
        public override DuplicatedInfo<InventoryOtherShipDocType> SetDuplicatedCheck()
        {
            return null;
        ...
        public void SyncData(string code)
        {
            ...
            U9Return<List<InventoryOtherShipDocType>> u9Return = apiHelper.GetMiscShipDocTypes(code);
            if (u9Return.Success)
            {
                ...
                {
                    List<InventoryOtherShipDocType> originals = DC.Set<InventoryOtherShipDocType>().Where(x => string.IsNullOrEmpty(code) || x.Code.Equals(code) || x.SourceSystemId.Equals(code)).AsNoTracking().ToList();
                    using (var trans = DC.BeginTransaction())
                    {
                        ...
                        foreach (var syncEntity in u9Return.Entity)
                        {
                            ...
                            InventoryOtherShipDocType match = null;
