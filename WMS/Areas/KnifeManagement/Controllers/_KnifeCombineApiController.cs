...
namespace WMS.KnifeManagement.Controllers
{
    ...
    [ActionDescription("_Model.KnifeCombine")]
    public partial class KnifeCombineController : BaseController
    {
        #region Create
        [HttpPost]
        [ActionDescription("Sys.Create")]
        public async Task<ActionResult> Create(KnifeCombineVM vm)
        {
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    vm.DoReInit();
                    return PartialView("../KnifeCombine/Create", vm);
                }
                else
                {
        ...
        public async Task<ActionResult> Edit(KnifeCombineVM vm)
        {
            if (!ModelState.IsValid)
            {
            ...
            else
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    vm.DoReInit();
                    return PartialView("../KnifeCombine/Edit", vm);
                }
                else
                {
        ...
        public ActionResult DoBatchEdit(KnifeCombineBatchVM vm, IFormCollection nouse)
        {
        ...
        {
            var vm = Wtm.CreateVM<KnifeCombineBatchVM>();
        ...
        public ActionResult Import(KnifeCombineImportVM vm, IFormCollection nouse)
        {
        ...
        public ActionResult Select_GetKnifeCombineByFrameworkUser(List<string> id)
        {
            var rv = DC.Set<KnifeCombine>().CheckIDs(id, x => x.HandledById).GetSelectListItems(Wtm,x=>x.DocNo);
            return JsonMore(rv);
        }
