...
{
    public partial class InventoryTransferOutManualApiVM : BaseCRUDVM<InventoryTransferOutManual>
    {
        #region 原生代码

        public InventoryTransferOutManualApiVM()
        {
        ...
        /// <returns></returns>
        public List<InventoryTransferOutManualReturn> GetList(int type = 0)
        {
            ...
            List<InventoryTransferOutManualReturn> list = new List<InventoryTransferOutManualReturn>();
            Expression<Func<InventoryTransferOutManual, bool>> queryable;
            if (type == 0)  // 待下架（未全部下架）。需匹配登录用户的存储地点
            {
                queryable = x => (x.Status == InventoryTransferOutManualStatusEnum.InWh
                    || x.Status == InventoryTransferOutManualStatusEnum.PartOff)
                    && x.TransOutWhId == whid;
            }
            else
            {
                queryable = x => x.Status == InventoryTransferOutManualStatusEnum.AllOff
                    && x.TransOutWhId == whid;
            }
            list = DC.Set<InventoryTransferOutManual>()
               .Where(queryable)
               .AsNoTracking()
               .Select(x => new InventoryTransferOutManualReturn
               {
                   ID = x.ID,
                   DocNo = x.DocNo,
                   DocType = x.DocType,
                   ...
                   SumQty = (int)(x.InventoryTransferOutManualLine_InventoryTransferOutManual.Sum(y => y.Qty) ?? 0),
        ...
        public void Create(InventoryTransferOutManual entity)
        {
            MSD.Clear();    // 看下是否需要判断
            var existEntity = DC.Set<InventoryTransferOutManual>().Where(x => x.DocNo == entity.DocNo).FirstOrDefault();
            if (existEntity != null)
            {
            ...
            r = this.SSIdAttrToId(entity, entity.TransOutOrganization, "TransOutOrganization");   // 调出组织
            if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return; }
            r = this.SSIdAttrToId(entity, entity.TransInOrganization, "TransInOrganization");   // 调入组织
            if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return; }
            r = this.SSIdAttrToId(entity, entity.TransOutWh, "TransOutWh");   // 调出存储地点
            if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return; }
            r = this.SSIdAttrToId(entity, entity.TransInWh, "TransInWh");   // 调入存储地点
            if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return; }
            foreach (var line in entity.InventoryTransferOutManualLine_InventoryTransferOutManual)
            {
            ...
            foreach (var line in entity.InventoryTransferOutManualLine_InventoryTransferOutManual)
            {
                line.Status = Model.InventoryTransferOutManualLineStatusEnum.InWh;
            ...
            // 将单据保存到本系统中
            Entity = entity;
            Entity.Status = Model.InventoryTransferOutManualStatusEnum.InWh;
            DoAdd();
        }
        ...
        public InventoryTransferOutManualReturn GetDoc(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            InventoryTransferOutManual doc = DC.Set<InventoryTransferOutManual>()
            ...
            {
                // 判断调出存储地点是否匹配当前登录用户的存储地点
                if (doc.TransOutWhId != whid)
                {
                    MSD.AddModelError("", $"单据{docNo}的调出存储地点为{doc.TransOutWh.Code}，与当前登录用户的存储地点{loginWh.Code}不匹配");
                    return null;
                }
                // 判断单据状态：只有在库和部分下架的才允许操作
                if (doc.Status != Model.InventoryTransferOutManualStatusEnum.InWh && doc.Status != Model.InventoryTransferOutManualStatusEnum.PartOff)
                {
                    MSD.AddModelError("", $"当前单据状态“{doc.Status.GetEnumDisplayName()}”不允许进行下架操作");
                    return null;
                }
                return new InventoryTransferOutManualReturn(doc);
            }
            // 本系统不存在时，尝试从ERP获取
            ...
            U9Return<InventoryTransferOutManual> u9Return = apiHelper.GetTransferOut(docNo);
            if (u9Return.Success)
            {
                // 引用类型外部系统ID转换
                ...
                r = this.SSIdAttrToId(u9Return.Entity, u9Return.Entity.TransOutOrganization, "TransOutOrganization");   // 调出组织
                if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                r = this.SSIdAttrToId(u9Return.Entity, u9Return.Entity.TransInOrganization, "TransInOrganization");   // 调入组织
                if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                r = this.SSIdAttrToId(u9Return.Entity, u9Return.Entity.TransOutWh, "TransOutWh");   // 调出存储地点
                if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                r = this.SSIdAttrToId(u9Return.Entity, u9Return.Entity.TransInWh, "TransInWh");   // 调入存储地点
                if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                foreach (var line in u9Return.Entity.InventoryTransferOutManualLine_InventoryTransferOutManual)
                {
                    r = this.SSIdAttrToId(line, line.ItemMaster, "ItemMaster");   // 转换物料
                    if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                }
                // 判断调出存储地点是否匹配（转换完了才能进行匹配，不然没有值）
                if (u9Return.Entity.TransOutWhId != whid)
                {
                    MSD.AddModelError("", $"单据{docNo}的调出存储地点与当前登录用户的存储地点{loginWh.Code}不匹配");
                    return null;
                }
                foreach (var line in u9Return.Entity.InventoryTransferOutManualLine_InventoryTransferOutManual)
                {
                ...
                doc = DC.Set<InventoryTransferOutManual>()
                    .Where(x => x.DocNo == docNo)
                    .Include(x => x.Organization)
                    .Include(x => x.TransInOrganization)
                    .Include(x => x.TransInWh)
        ...
        /// <returns></returns>
        public InventoryTransferOutManualReturn GetDocForApprove(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                if (doc.Status != InventoryTransferOutManualStatusEnum.AllOff)
                {
                    MSD.AddModelError("", $"单据{docNo}的状态为{doc.Status.GetEnumDisplayName()}，无法进行审核");
                    return null;
                }
                InventoryTransferOutManualReturn result = new InventoryTransferOutManualReturn(doc);
        ...
        public void GetOffDetails(InventoryTransferOutManualReturn doc)
        {
            var invRelations = DC.Set<BaseDocInventoryRelation>().AsNoTracking().Include(x => x.Inventory.WhLocation.WhArea.WareHouse)
                .Where(x => x.BusinessId == doc.ID && x.DocType == DocTypeEnum.InventoryTransferOutManual).ToList();
            ...
            {
                line.SubLines = new List<InventoryTransferOutManualReturnSubLine>();
                invRelations.FindAll(x => x.BusinessLineId == line.ID).ForEach(x =>
                {
                    ...
                    //    return;
                    //}
                    line.SubLines.Add(new InventoryTransferOutManualReturnSubLine
                    {
        ...
        /// </summary>
        /// <param name="doc"></param>
        public void GetSuggestLocs(InventoryTransferOutManualReturn doc)
        {
            foreach (var line in doc.Lines)
            {
                line.SuggestLocs = new List<InventoryTransferOutManualReturnLineSuggestLoc>();
                List<BaseInventory> invs = this.GetItemInventory(line.ItemID, doc.TransOutWhId);
                if (invs.Count > 0)
                {
                    // invs = invs.OrderBy(x => x.CreateTime).ToList(); // 默认升序
                    foreach (var inv in invs)
                    {
                        line.SuggestLocs.Add(new InventoryTransferOutManualReturnLineSuggestLoc
                        {
        ...
        {
            var entity = DC.Set<InventoryTransferOutManual>().Where(x => x.SourceSystemId == sourceSystemId).AsNoTracking().FirstOrDefault();
            if (entity == null)
            {
                return false;
            }
            ...
            {
                if (entity.Status == InventoryTransferOutManualStatusEnum.InWh)
                {
        ...
        /// </summary>
        /// <param name="doc"></param>
        public void SaveOffDetails(InventoryTransferOutManualSaveOffPara data)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            using (var trans = DC.BeginTransaction())
            {
                try
                {
                    InventoryTransferOutManual doc = DC.Set<InventoryTransferOutManual>()             //.AsNoTracking()
                        .Include(x => x.InventoryTransferOutManualLine_InventoryTransferOutManual)
                    ...
                    if (doc.Status != InventoryTransferOutManualStatusEnum.InWh && doc.Status != InventoryTransferOutManualStatusEnum.PartOff)
                    {
                    ...
                    {
                        var docline = doc.InventoryTransferOutManualLine_InventoryTransferOutManual.Find(x => x.ID == dataLine.ID);
                        if (docline == null)
                        {
                            MSD.AddModelError("", $"行数据“{dataLine.ID}”不存在");
                            return;
                        }
                        if (docline.Status != InventoryTransferOutManualLineStatusEnum.InWh && docline.Status != InventoryTransferOutManualLineStatusEnum.PartOff)
                        {
                        ...
                        if (docline.ToBeOffQty == 0)
                        {
                            docline.Status = InventoryTransferOutManualLineStatusEnum.AllOff;
                        }
                        else if (docline.OffQty == 0)
                        {
                            docline.Status = InventoryTransferOutManualLineStatusEnum.InWh;
                        }
                        else
                        {
                            docline.Status = InventoryTransferOutManualLineStatusEnum.PartOff;
                        }
                    }
                    ...
                    // 设置单据状态
                    if (doc.InventoryTransferOutManualLine_InventoryTransferOutManual.All(x => x.Status == InventoryTransferOutManualLineStatusEnum.AllOff))   // 全部行已下架
                    {
                        doc.Status = InventoryTransferOutManualStatusEnum.AllOff;
                    }
                    else if (doc.InventoryTransferOutManualLine_InventoryTransferOutManual.All(x => x.Status == InventoryTransferOutManualLineStatusEnum.InWh))  // 全部行未下架（下架动作不会出现这种情况）
                    {
                        doc.Status = InventoryTransferOutManualStatusEnum.InWh;
                    }
                    else
                    {
                        doc.Status = InventoryTransferOutManualStatusEnum.PartOff;
                    }
                    DC.SaveChanges();
                    ...
                    {
                        // 创建库存流水
                        if (!this.CreateInvLog(OperationTypeEnum.InventoryTransferOutManualOff, doc.DocNo, sourceInvIds[i], newInventoryList[i].ID, -newInventoryList[i].Qty, newInventoryList[i].Qty))
                        {
                            return;
                        }
                    }
                    ...
                    {
                        // 创建单据库存关联关系
                        if (!this.CreateInvRelation(DocTypeEnum.InventoryTransferOutManual, item.Item2, doc.ID, item.Item1, item.Item3))
                        {
                            return;
                        }
                    }
        ...
        public void CancelOff(Guid? docId)
        {
            ...
            try
            {
                using (var trans = DC.BeginTransaction())
                {
                    var doc = DC.Set<InventoryTransferOutManual>().Include(x => x.InventoryTransferOutManualLine_InventoryTransferOutManual).Where(x => x.ID == docId).FirstOrDefault();
                    if (doc == null)
                    {
                        MSD.AddModelError("", "单据不存在");
                        return;
                    }
                    if (doc.Status == InventoryTransferOutManualStatusEnum.InWh)
                    {
                        MSD.AddModelError("", "单据尚未进行下架业务，无法进行取消下架操作");
                        return;
                    }
                    if (doc.Status == InventoryTransferOutManualStatusEnum.PartShipped || doc.Status == InventoryTransferOutManualStatusEnum.AllShipped)
                    {
                        MSD.AddModelError("", "单据已出货，无法进行取消下架操作");
                        return;
                    }
                    // 取消下架
                    foreach (var line in doc.InventoryTransferOutManualLine_InventoryTransferOutManual)
                    {
                        if (line.Status == InventoryTransferOutManualLineStatusEnum.InWh)
                        {
                            continue;
                        }
                        if (line.OffQty <= 0)
                        ...
                        else if (line.Status == InventoryTransferOutManualLineStatusEnum.PartShipped || line.Status == InventoryTransferOutManualLineStatusEnum.AllShipped) // 头部已过滤，不会出现这种情况
                        {
                        ...
                        line.ToBeShipQty = 0;
                        line.Status = InventoryTransferOutManualLineStatusEnum.InWh;
                    }
                    // 设置单据状态
                    if (doc.InventoryTransferOutManualLine_InventoryTransferOutManual.All(x => x.Status == InventoryTransferOutManualLineStatusEnum.AllOff))   // 全部行已下架（取消下架动作不会出现这种情况）
                    {
                        doc.Status = InventoryTransferOutManualStatusEnum.AllOff;
                    }
                    else if (doc.InventoryTransferOutManualLine_InventoryTransferOutManual.All(x => x.Status == InventoryTransferOutManualLineStatusEnum.InWh))  // 全部行未下架
                    {
                        doc.Status = InventoryTransferOutManualStatusEnum.InWh;
                    }
                    else
                    ...
                    foreach (var relation in invRelations)
                    {
                        ...
                        // 创建库存流水
                        if (!this.CreateInvLog(OperationTypeEnum.InventoryTransferOutManualOffCancel, doc.DocNo, relation.InventoryId, originalInv.ID, -inventory.Qty, inventory.Qty))
                        {
                            return;
                        }
                        // 待发库位的库存信息清零
        ...
        public void Approve(Guid? docId)
        {
            ...
            using (var trans = DC.BeginTransaction())
            {
                var doc = DC.Set<InventoryTransferOutManual>().Include(x => x.InventoryTransferOutManualLine_InventoryTransferOutManual).ThenInclude(x => x.ItemMaster).Include(x => x.Organization).Where(x => x.ID == docId).FirstOrDefault();
                if (doc == null)
                {
                    MSD.AddModelError("", "单据不存在");
                    return;
                ...
                // 传U9接口的参数
                ApproveTransferOutPara para = new ApproveTransferOutPara
                {
                    docNo = doc.DocNo,
                    businessId = doc.ID,
                    orgCode = doc.Organization?.Code,
                    transOutLineDTOList = new List<ApproveTransferOutLinePara>(),
                };
                ...
                foreach (var line in doc.InventoryTransferOutManualLine_InventoryTransferOutManual)
                {
                    line.Status = InventoryTransferOutManualLineStatusEnum.AllShipped;
                    ...
                    foreach (var relation in matchRelations)
                    {
                        ...
                        // 创建库存流水
                        this.CreateInvLog(OperationTypeEnum.InventoryTransferOutManualApprove, doc.DocNo, inventory.ID, null, -relation.Qty, null);
                ...
                doc.Status = InventoryTransferOutManualStatusEnum.AllShipped;
                ...
                U9Return u9Return = apiHelper.ApproveTransferOut(para);
                if (u9Return.Success)
                {
                    trans.Commit();
                }
