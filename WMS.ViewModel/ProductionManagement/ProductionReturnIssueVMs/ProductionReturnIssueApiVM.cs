...
{
    public partial class ProductionReturnIssueApiVM : BaseCRUDVM<ProductionReturnIssue>
    {
        #region 系统标准方法

        public ProductionReturnIssueApiVM()
        {
        ...
        /// <returns></returns>
        public List<ProductionReturnIssueReturn> GetList(int type = 0)
        {
            ...
            List<ProductionReturnIssueReturn> list = new List<ProductionReturnIssueReturn>();
            Expression<Func<ProductionReturnIssue, bool>> queryable;
            if (type == 0)  // 待收货（未全部收货）。需匹配登录用户的存储地点
            {
                queryable = x => (x.Status == Model.ProductionReturnIssueStatusEnum.NotReceive
                    || x.Status == Model.ProductionReturnIssueStatusEnum.PartReceive)
                    && x.ProductionReturnIssueLine_ProductionReturnIssue.Any(y => y.WhId == whid);
            }
            else
            {
                queryable = x => x.Status == Model.ProductionReturnIssueStatusEnum.AllReceive
                    && x.ProductionReturnIssueLine_ProductionReturnIssue.Any(y => y.WhId == whid);
            }
            list = DC.Set<ProductionReturnIssue>()
               .Where(queryable)
               .AsNoTracking()
               .Select(x => new ProductionReturnIssueReturn
               {
                   ID = x.ID,
                   DocNo = x.DocNo,
                   DocType = x.DocType,
                   ...
                   SumQty = (int)(x.ProductionReturnIssueLine_ProductionReturnIssue.Sum(y => y.Qty) ?? 0),
        ...
        public ProductionReturnIssueReturn GetDoc(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            ProductionReturnIssue doc = DC.Set<ProductionReturnIssue>()
                .Where(x => x.DocNo == docNo)
                .Include(x => x.Organization)
                .Include(x => x.ProductionReturnIssueLine_ProductionReturnIssue)
                .ThenInclude(x => x.Wh)
                .Include(x => x.ProductionReturnIssueLine_ProductionReturnIssue)
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                ProductionReturnIssueLine line = doc.ProductionReturnIssueLine_ProductionReturnIssue.Find(x => x.WhId != whid);
                if (line != null)
                {
                    // 存在行存储地点不匹配，返回错误信息
                    MSD.AddModelError("", $"单据{docNo}的存储地点为{line.Wh.Code}，与当前登录用户的存储地点{loginWh.Code}不匹配");
                    return null;
                }
                if (doc.Status != Model.ProductionReturnIssueStatusEnum.NotReceive && doc.Status != Model.ProductionReturnIssueStatusEnum.PartReceive)
                {
                    MSD.AddModelError("", $"当前单据状态“{doc.Status.GetEnumDisplayName()}”，无法进行收货");
                    return null;
                }
                return new ProductionReturnIssueReturn(doc);
            }
            // 本系统不存在时，尝试从ERP获取
            ...
            U9Return<ProductionReturnIssue> u9Return = apiHelper.GetProductionReturnIssue(docNo);
            if (u9Return.Success)
            {
                // 引用类型外部系统ID转换
                ...
                foreach (var line in u9Return.Entity.ProductionReturnIssueLine_ProductionReturnIssue)
                {
                ...
                foreach (var line in u9Return.Entity.ProductionReturnIssueLine_ProductionReturnIssue)
                {
                    ...
                    line.Status = Model.ProductionReturnIssueLineStatusEnum.NotReceive;
                ...
                Entity.Status = Model.ProductionReturnIssueStatusEnum.NotReceive;
                DoAdd();
                if (!MSD.IsValid)
                {
                    return null;
                }
                doc = DC.Set<ProductionReturnIssue>()
                    .Where(x => x.DocNo == docNo)
                    .Include(x => x.Organization)
                    .Include(x => x.ProductionReturnIssueLine_ProductionReturnIssue)
                    .ThenInclude(x => x.Wh)
        ...
        /// <returns></returns>
        public ProductionReturnIssueReturn GetDocForApprove(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                //    return null;
                //}
                if (doc.Status != ProductionReturnIssueStatusEnum.AllReceive)
                {
                    MSD.AddModelError("", $"单据{docNo}的状态为{doc.Status.GetEnumDisplayName()}，无法进行审核");
                    return null;
                }
                ProductionReturnIssueReturn result = new ProductionReturnIssueReturn(doc);
                // 获取单据库存关联关系数据
        ...
        public void GetSuggestLocs(ProductionReturnIssueReturn doc)
        {
            foreach (var line in doc.Lines)
            {
                line.SuggestLocs = new List<ProductionReturnIssueReturnLineSuggestLoc>();
                ...
                if (invs.Count > 0)
                {
                    ...
                    foreach (var inv in invs)
                    {
                        ...
                        {
                            line.SuggestLocs.Add(new ProductionReturnIssueReturnLineSuggestLoc
                            {
        ...
        {
            var entity = DC.Set<ProductionReturnIssue>().Where(x => x.SourceSystemId == sourceSystemId).AsNoTracking().FirstOrDefault();
            if (entity == null)
            {
                return false;
            }
            ...
            {
                if (entity.Status == Model.ProductionReturnIssueStatusEnum.NotReceive)
                {
        ...
        public void Create(ProductionReturnIssue entity)
        {
            MSD.Clear();    // 看下是否需要判断
            var existEntity = DC.Set<ProductionReturnIssue>().Where(x => x.DocNo == entity.DocNo).FirstOrDefault();
            if (existEntity != null)
            {
        ...
        public void Receiving(ReceivingPara data)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            {
                ProductionReturnIssue doc = DC.Set<ProductionReturnIssue>()             //.AsNoTracking()
                    .Include(x => x.ProductionReturnIssueLine_ProductionReturnIssue)
                    .ThenInclude(x => x.ItemMaster)
                ...
                if (doc.ProductionReturnIssueLine_ProductionReturnIssue.Any(x => x.WhId != whid))
                {
                    MSD.AddModelError("", "单据存储地点不匹配，可尝试重新选择登录存储地点");
                    return;
                }
                if (doc.Status != ProductionReturnIssueStatusEnum.NotReceive && doc.Status != ProductionReturnIssueStatusEnum.PartReceive)
                {
                ...
                {
                    var line = doc.ProductionReturnIssueLine_ProductionReturnIssue.Find(x => x.ID == dataLine.ID);
                    if (line == null)
                    {
                        MSD.AddModelError("", $"行数据“{dataLine.ID}”不存在");
                        return;
                    }
                    if (line.Status != ProductionReturnIssueLineStatusEnum.NotReceive && line.Status != ProductionReturnIssueLineStatusEnum.PartReceive)
                    {
                    ...
                    if (line.ToBeReceiveQty == 0)
                    {
                        line.Status = ProductionReturnIssueLineStatusEnum.AllReceive;
                    }
                    else if (line.ReceiveQty == 0)
                    {
                        line.Status = ProductionReturnIssueLineStatusEnum.NotReceive;
                    }
                    else
                    {
                        line.Status = ProductionReturnIssueLineStatusEnum.PartReceive;
                    }
                }
                // 验证所有条码均被完全使用
                ...
                // 整行拒收的自动设置为已收货
                //foreach (var line in doc.ProductionReturnIssueLine_ProductionReturnIssue)
                //{
                //    if (line.Status == ProductionReturnIssueLineStatusEnum.NotReceive
                ...
                // 设置单据状态
                if (doc.ProductionReturnIssueLine_ProductionReturnIssue.All(x => x.Status == ProductionReturnIssueLineStatusEnum.AllReceive))   // 全部行已收货
                {
                    doc.Status = ProductionReturnIssueStatusEnum.AllReceive;
                }
                else if (doc.ProductionReturnIssueLine_ProductionReturnIssue.All(x => x.Status == ProductionReturnIssueLineStatusEnum.NotReceive))  // 全部行未收货（收货动作不会出现这种情况）
                {
                    doc.Status = ProductionReturnIssueStatusEnum.NotReceive;
                }
                else
                {
                    doc.Status = ProductionReturnIssueStatusEnum.PartReceive;
                }
                ...
                {
                    // 创建库存流水
                    if (!this.CreateInvLog(OperationTypeEnum.ProductionReturnIssueReceive, doc.DocNo, null, inventory.ID, null, inventory.Qty))
                    {
                        return;
                    }
                }
                ...
                {
                    // 创建单据库存关联关系
                    if (!this.CreateInvRelation(DocTypeEnum.ProductionReturnIssue, item.Item2, doc.ID, item.Item1, item.Item3))  // 采购收货单收货操作
                    {
                        return;
                    }
                }
        ...
        public void CancelReceive(Guid? docId)
        {
            ...
            using (var trans = DC.BeginTransaction())
            {
                var doc = DC.Set<ProductionReturnIssue>().Include(x => x.ProductionReturnIssueLine_ProductionReturnIssue).Where(x => x.ID == docId).FirstOrDefault();
                if (doc == null)
                {
                    MSD.AddModelError("", "单据不存在");
                    return;
                }
                if (doc.Status == ProductionReturnIssueStatusEnum.NotReceive)
                {
                    MSD.AddModelError("", "单据尚未进行收货业务，无法进行取消收货操作");
                    return;
                }
                if (doc.Status == ProductionReturnIssueStatusEnum.AllInWh || doc.Status == ProductionReturnIssueStatusEnum.PartInWh)
                {
                ...
                foreach (var line in doc.ProductionReturnIssueLine_ProductionReturnIssue)
                {
                    ...
                    else if (line.Status == ProductionReturnIssueLineStatusEnum.PartInWh || line.Status == ProductionReturnIssueLineStatusEnum.AllInWh) // 不会出现。当前设置逻辑为必须一次性全部入库。单据头判断已经进行了过滤
                    {
                ...
                // 设置单据状态
                if (doc.ProductionReturnIssueLine_ProductionReturnIssue.All(x => x.Status == ProductionReturnIssueLineStatusEnum.AllReceive))   // 全部行已收货（取消收货动作不会出现这种情况）
                {
                    doc.Status = ProductionReturnIssueStatusEnum.AllReceive;
                }
                else if (doc.ProductionReturnIssueLine_ProductionReturnIssue.All(x => x.Status == ProductionReturnIssueLineStatusEnum.NotReceive))  // 全部行未收货
                {
                    doc.Status = ProductionReturnIssueStatusEnum.NotReceive;
                }
                else
                ...
                var invRelations = DC.Set<BaseDocInventoryRelation>()
                    .Where(x => x.BusinessId == doc.ID && x.DocType == DocTypeEnum.ProductionReturnIssue).ToList();
                ...
                foreach (var relation in invRelations)
                {
                    ...
                    // 创建库存流水
                    if (!this.CreateInvLog(OperationTypeEnum.ProductionReturnIssueReceiveCancel, doc.DocNo, relation.InventoryId, null, -inventory.Qty, null))
                    {
                        return;
                    }
                    // 待收库位的库存信息清零
        ...
        /// </summary>
        /// <param name="docId"></param>
        public void ApproveAndPutaway(ReceivingApprovePara data)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            using (var trans = DC.BeginTransaction())
            {
                var doc = DC.Set<ProductionReturnIssue>()
                    .Include(x => x.ProductionReturnIssueLine_ProductionReturnIssue)
                    .Include(x => x.Organization)
                    .Where(x => x.ID == data.ID)
                    .FirstOrDefault();
                ...
                var invIds = DC.Set<BaseDocInventoryRelation>()
                    .Where(x => x.BusinessId == doc.ID && x.DocType == DocTypeEnum.ProductionReturnIssue)
                    .AsNoTracking()
                ...
                foreach (var item in data.Details)
                {
                    ...
                    // 创建库存流水
                    this.CreateInvLog(OperationTypeEnum.ProductionReturnIssueApprove, doc.DocNo, oldInv.ID, newInv.ID, -newInv.Qty, newInv.Qty);
                }
                // 更新单据状态、行状态
                doc.Status = ProductionReturnIssueStatusEnum.AllInWh;
                doc.ProductionReturnIssueLine_ProductionReturnIssue.ForEach(x =>
                {
                    x.Status = ProductionReturnIssueLineStatusEnum.AllInWh;
                ...
                U9Return u9Return = apiHelper.ApproveProductionIssue(doc.DocNo);    // 领退料是同一个审核接口（U9同实体）
                if (u9Return.Success)
                {
                    trans.Commit();
                }
