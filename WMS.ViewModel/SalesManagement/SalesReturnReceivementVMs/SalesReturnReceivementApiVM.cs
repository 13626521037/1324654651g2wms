...
{
    public partial class SalesReturnReceivementApiVM : BaseCRUDVM<SalesReturnReceivement>
    {
        #region 原生代码

        public SalesReturnReceivementApiVM()
        {
        ...
        public void Create(SalesReturnReceivement entity)
        {
            MSD.Clear();    // 看下是否需要判断
            var existEntity = DC.Set<SalesReturnReceivement>().Where(x => x.DocNo == entity.DocNo).FirstOrDefault();
            if (existEntity != null)
            {
            ...
            if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return; }
            foreach (var line in entity.SalesReturnReceivementLine_ReturnReceivement)
            {
            ...
            foreach (var line in entity.SalesReturnReceivementLine_ReturnReceivement)
            {
                line.Status = Model.SalesReturnReceivementLineStatusEnum.NotReceive;
            ...
            // 将单据保存到本系统中
            Entity = entity;
            Entity.Status = Model.SalesReturnReceivementStatusEnum.NotReceive;
            DoAdd();
        }
        ...
        {
            var entity = DC.Set<SalesReturnReceivement>().Where(x => x.SourceSystemId == sourceSystemId).AsNoTracking().FirstOrDefault();
            if (entity == null)
            {
                return false;
            }
        ...
        public SalesReturnReceivementReturn GetDoc(string docNo)
        {
            ...
            SalesReturnReceivement doc = DC.Set<SalesReturnReceivement>()
                .Where(x => x.DocNo == docNo || x.SalesReturnReceivementLine_ReturnReceivement.Any(y => y.SrcDocNo == docNo))
                .Include(x => x.Organization)
                .Include(x => x.Customer)
                .Include(x => x.SalesReturnReceivementLine_ReturnReceivement)
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                SalesReturnReceivementLine line = doc.SalesReturnReceivementLine_ReturnReceivement.Find(x => x.WareHouseId != whid);
                if (line != null)
                {
                    // 存在行存储地点不匹配，返回错误信息
                ...
                if (doc.Status != SalesReturnReceivementStatusEnum.NotReceive && doc.Status != SalesReturnReceivementStatusEnum.PartReceive)
                {
                    MSD.AddModelError("", $"当前单据状态“{doc.Status.GetEnumDisplayName()}”，无法进行收货");
                    return null;
                }
                return new SalesReturnReceivementReturn(doc);
            }
            // 本系统不存在时，尝试从ERP获取
            ...
            U9Return<SalesReturnReceivement> u9Return = apiHelper.GetReturnRcv(docNo);
            if (u9Return.Success)
            {
                // 引用类型外部系统ID转换
                ...
                r = this.SSIdAttrToId(u9Return.Entity, u9Return.Entity.Customer, "Customer");   // 转换客户
                if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                foreach (var line in u9Return.Entity.SalesReturnReceivementLine_ReturnReceivement)
                {
                    r = this.SSIdAttrToId(line, line.ItemMaster, "ItemMaster");   // 转换物料
                    if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                    r = this.SSIdAttrToId(line, line.WareHouse, "WareHouse");   // 转换存储地点
                ...
                foreach (var line in u9Return.Entity.SalesReturnReceivementLine_ReturnReceivement)
                {
                    ...
                    line.Status = SalesReturnReceivementLineStatusEnum.NotReceive;
                    line.ToBeReceiveQty = line.Qty;
                    line.ToBeInWhQty = 0;
                    line.ReceiveQty = 0;
                    line.InWhQty = 0;
                ...
                Entity.Status = SalesReturnReceivementStatusEnum.NotReceive;
                DoAdd();
                if (!MSD.IsValid)
                {
                    return null;
                }
                doc = DC.Set<SalesReturnReceivement>()
                    .Where(x => x.DocNo == docNo || x.SalesReturnReceivementLine_ReturnReceivement.Any(y => y.SrcDocNo == docNo))
                    .Include(x => x.Organization)
                    .Include(x => x.Customer)
                    .Include(x => x.SalesReturnReceivementLine_ReturnReceivement)
        ...
        /// <returns></returns>
        public SalesReturnReceivementReturn GetDocForApprove(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                //    return null;
                //}
                if (doc.Status != SalesReturnReceivementStatusEnum.AllReceive)
                {
                    MSD.AddModelError("", $"单据{docNo}的状态为{doc.Status.GetEnumDisplayName()}，无法进行审核");
                    return null;
                }
                SalesReturnReceivementReturn result = new SalesReturnReceivementReturn(doc);
                // 获取单据库存关联关系数据
        ...
        public void GetSuggestLocs(SalesReturnReceivementReturn doc)
        {
            foreach (var line in doc.Lines)
            {
                line.SuggestLocs = new List<SalesReturnReceivementReturnLineSuggestLoc>();
                ...
                if (invs.Count > 0)
                {
                    ...
                    foreach (var inv in invs)
                    {
                        ...
                        {
                            line.SuggestLocs.Add(new SalesReturnReceivementReturnLineSuggestLoc
                            {
        ...
        /// <returns></returns>
        public List<SalesReturnReceivementReturn> GetList(int type = 0)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            List<SalesReturnReceivementReturn> list = new List<SalesReturnReceivementReturn>();
            Expression<Func<SalesReturnReceivement, bool>> queryable;
            if (type == 0)  // 待收货（未全部收货）。需匹配登录用户的存储地点
            {
                queryable = x => (x.Status == SalesReturnReceivementStatusEnum.NotReceive
                    || x.Status == SalesReturnReceivementStatusEnum.PartReceive)
                    && x.SalesReturnReceivementLine_ReturnReceivement.Any(y => y.WareHouseId == whid);
            }
            else
            {
                queryable = x => x.Status == SalesReturnReceivementStatusEnum.AllReceive
                    && x.SalesReturnReceivementLine_ReturnReceivement.Any(y => y.WareHouseId == whid);
            }
            list = DC.Set<SalesReturnReceivement>()
               .Where(queryable)
               .AsNoTracking()
               .Select(x => new SalesReturnReceivementReturn
               {
                   ID = x.ID,
                   DocNo = x.DocNo,
                   DocType = x.DocType,
                   ...
                   SumQty = (int)(x.SalesReturnReceivementLine_ReturnReceivement.Sum(y => y.Qty) ?? 0),
        ...
        public void Receiving(SalesReturnReceivementReceivingPara data)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            {
                SalesReturnReceivement doc = DC.Set<SalesReturnReceivement>()             //.AsNoTracking()
                    .Include(x => x.SalesReturnReceivementLine_ReturnReceivement)
                    .ThenInclude(x => x.ItemMaster)
                ...
                if (doc.SalesReturnReceivementLine_ReturnReceivement.Any(x => x.WareHouseId != whid))
                {
                ...
                {
                    var line = doc.SalesReturnReceivementLine_ReturnReceivement.Find(x => x.ID == dataLine.ID);
                    if (line == null)
                    {
                        MSD.AddModelError("", $"行数据“{dataLine.ID}”不存在");
                        return;
                    }
                    if (line.Status != SalesReturnReceivementLineStatusEnum.NotReceive && line.Status != SalesReturnReceivementLineStatusEnum.PartReceive)
                    {
                    ...
                    foreach (var dataSubLine in dataLine.SubLines)
                    {
                        ...
                        if (barcode.Seiban != line.Seiban)
                        {
                            MSD.AddModelError("", $"行“{line.DocLineNo}”，子行“{barcode.Code}”条码与行的番号不匹配");
                            return;
                        }
                    ...
                    if (line.ToBeReceiveQty == 0)
                    {
                        line.Status = SalesReturnReceivementLineStatusEnum.AllReceive;
                    }
                    ...
                    {
                        line.Status = SalesReturnReceivementLineStatusEnum.PartReceive;
                    }
                }
                // 验证所有条码均被完全使用
                ...
                // 设置单据状态
                if (doc.SalesReturnReceivementLine_ReturnReceivement.All(x => x.Status == SalesReturnReceivementLineStatusEnum.AllReceive))   // 全部行已收货
                {
                    doc.Status = SalesReturnReceivementStatusEnum.AllReceive;
                }
                else if (doc.SalesReturnReceivementLine_ReturnReceivement.All(x => x.Status == SalesReturnReceivementLineStatusEnum.NotReceive))  // 全部行未收货（收货动作不会出现这种情况）
                {
                    doc.Status = SalesReturnReceivementStatusEnum.NotReceive;
                }
                else
                {
                    doc.Status = SalesReturnReceivementStatusEnum.PartReceive;
                }
                ...
                {
                    // 创建库存流水
                    if (!this.CreateInvLog(OperationTypeEnum.ReturnReceivementReceive, doc.DocNo, null, inventory.ID, null, inventory.Qty))
                    {
                        return;
                    }
                }
                ...
                {
                    // 创建单据库存关联关系
                    if (!this.CreateInvRelation(DocTypeEnum.ReturnReceivement, item.Item2, doc.ID, item.Item1, item.Item3))
                    {
                        return;
                    }
                }
        ...
        public void CancelReceive(Guid? docId)
        {
            ...
            using (var trans = DC.BeginTransaction())
            {
                var doc = DC.Set<SalesReturnReceivement>().Include(x => x.SalesReturnReceivementLine_ReturnReceivement).Where(x => x.ID == docId).FirstOrDefault();
                if (doc == null)
                {
                    MSD.AddModelError("", "单据不存在");
                    return;
                }
                if (doc.Status == SalesReturnReceivementStatusEnum.NotReceive)
                {
                    MSD.AddModelError("", "单据尚未进行收货业务，无法进行取消收货操作");
                    return;
                }
                if (doc.Status == SalesReturnReceivementStatusEnum.AllInWh || doc.Status == SalesReturnReceivementStatusEnum.PartInWh)
                {
                ...
                //List<BaseInventory> newInventoryList = new List<BaseInventory>();
                // 取消收货
                foreach (var line in doc.SalesReturnReceivementLine_ReturnReceivement)
                {
                    if (line.Status == SalesReturnReceivementLineStatusEnum.NotReceive)
                    {
                    ...
                    else if (line.Status == SalesReturnReceivementLineStatusEnum.PartInWh || line.Status == SalesReturnReceivementLineStatusEnum.AllInWh) // 不会出现。当前设置逻辑为必须一次性全部入库。单据头判断已经进行了过滤
                    {
                ...
                // 设置单据状态
                if (doc.SalesReturnReceivementLine_ReturnReceivement.All(x => x.Status == SalesReturnReceivementLineStatusEnum.AllReceive))   // 全部行已收货（取消收货动作不会出现这种情况）
                {
                    doc.Status = SalesReturnReceivementStatusEnum.AllReceive;
                }
                else if (doc.SalesReturnReceivementLine_ReturnReceivement.All(x => x.Status == SalesReturnReceivementLineStatusEnum.NotReceive))  // 全部行未收货
                {
                    doc.Status = SalesReturnReceivementStatusEnum.NotReceive;
                }
                else
                ...
                var invRelations = DC.Set<BaseDocInventoryRelation>()
                    .Where(x => x.BusinessId == doc.ID && x.DocType == DocTypeEnum.ReturnReceivement).ToList();
                ...
                foreach (var relation in invRelations)
                {
                    ...
                    // 创建库存流水
                    if (!this.CreateInvLog(OperationTypeEnum.ReturnReceivementReceiveCancel, doc.DocNo, relation.InventoryId, null, -inventory.Qty, null))
                    {
                        return;
                    }
                    // 待收库位的库存信息清零
        ...
        /// </summary>
        /// <param name="docId"></param>
        public void ApproveAndPutaway(SalesReturnReceivementApprovePara data)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            using (var trans = DC.BeginTransaction())
            {
                var doc = DC.Set<SalesReturnReceivement>()
                    .Include(x => x.SalesReturnReceivementLine_ReturnReceivement)
                    .Include(x => x.Organization)
                    .Where(x => x.ID == data.ID)
                    .FirstOrDefault();
                ...
                var invIds = DC.Set<BaseDocInventoryRelation>()
                    .Where(x => x.BusinessId == doc.ID && x.DocType == DocTypeEnum.ReturnReceivement)
                    .AsNoTracking()
                ...
                foreach (var item in data.Details)
                {
                    ...
                    // 创建库存流水
                    this.CreateInvLog(OperationTypeEnum.ReturnReceivementApprove, doc.DocNo, oldInv.ID, newInv.ID, -newInv.Qty, newInv.Qty);
                }
                // 更新单据状态、行状态
                doc.Status = SalesReturnReceivementStatusEnum.AllInWh;
                doc.SalesReturnReceivementLine_ReturnReceivement.ForEach(x =>
                {
                    x.Status = SalesReturnReceivementLineStatusEnum.AllInWh;
