...
{
    public partial class PurchaseReturnApiVM : BaseCRUDVM<PurchaseReturn>
    {
        /// <summary>
        /// 自动拆码结果
        /// </summary>
        ...
        /// <returns></returns>
        public List<PurchaseReturnReturn> GetList(int type = 0)
        {
            ...
            List<PurchaseReturnReturn> list = new List<PurchaseReturnReturn>();
            Expression<Func<PurchaseReturn, bool>> queryable;
            if (type == 0)  // 待下架（未全部下架）。需匹配登录用户的存储地点
            {
                queryable = x => (x.Status == PurchaseReturnStatusEnum.InWh
                    || x.Status == PurchaseReturnStatusEnum.PartOff)
                    && x.PurchaseReturnLine_PurchaseReturn.Any(y => y.WareHouseId == whid);
            }
            else
            {
                queryable = x => x.Status == PurchaseReturnStatusEnum.AllOff 
                    && x.PurchaseReturnLine_PurchaseReturn.Any(y => y.WareHouseId == whid);
            }
            list = DC.Set<PurchaseReturn>()
               .Where(queryable)
               .AsNoTracking()
               .Select(x => new PurchaseReturnReturn
               {
                   ID = x.ID,
                   DocNo = x.DocNo,
                   DocType = x.DocType,
                   ...
                   SumQty = (int)(x.PurchaseReturnLine_PurchaseReturn.Sum(y => y.Qty) ?? 0),
        ...
        public PurchaseReturnReturn GetDoc(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            PurchaseReturn doc = DC.Set<PurchaseReturn>()
                .Where(x => x.DocNo == docNo)
                .Include(x => x.Organization)
                .Include(x => x.Supplier)
                .Include(x => x.PurchaseReturnLine_PurchaseReturn)
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                PurchaseReturnLine line = doc.PurchaseReturnLine_PurchaseReturn.Find(x => x.WareHouseId != whid);
                if (line != null)
                {
                    // 存在行存储地点不匹配，返回错误信息
                ...
                if (doc.Status != Model.PurchaseReturnStatusEnum.InWh && doc.Status != Model.PurchaseReturnStatusEnum.PartOff)
                {
                    MSD.AddModelError("", $"当前单据状态“{doc.Status.GetEnumDisplayName()}”不允许进行下架操作");
                    return null;
                }
                return new PurchaseReturnReturn(doc);
            }
            // 本系统不存在时，尝试从ERP获取
            ...
            U9Return<PurchaseReturn> u9Return = apiHelper.GetPurchaseReturn(docNo);
            if (u9Return.Success)
            {
                // 引用类型外部系统ID转换
                ...
                if (!string.IsNullOrEmpty(r)) { MSD.AddModelError("", r); return null; }
                foreach (var line in u9Return.Entity.PurchaseReturnLine_PurchaseReturn)
                {
                ...
                foreach (var line in u9Return.Entity.PurchaseReturnLine_PurchaseReturn)
                {
                    ...
                    line.Status = Model.PurchaseReturnLineStatusEnum.InWh;
                ...
                Entity.Status = Model.PurchaseReturnStatusEnum.InWh;
                DoAdd();
                if (!MSD.IsValid)
                {
                    return null;
                }
                doc = DC.Set<PurchaseReturn>()
                    .Where(x => x.DocNo == docNo)
                    .Include(x => x.Organization)
                    .Include(x => x.Supplier)
                    .Include(x => x.PurchaseReturnLine_PurchaseReturn)
        ...
        /// <returns></returns>
        public PurchaseReturnReturn GetDocForApprove(string docNo)
        {
            if (!LoginUserInfo.Attributes.HasKey("WarehouseId") || LoginUserInfo.Attributes["WarehouseId"] == null)
            {
                MSD.AddModelError("", "登录信息已过期，请重新登录");
            ...
            if (doc != null)    // 单据在本系统中存在
            {
                ...
                if (doc.Status != PurchaseReturnStatusEnum.AllOff)
                {
                    MSD.AddModelError("", $"单据{docNo}的状态为{doc.Status.GetEnumDisplayName()}，无法进行审核");
                    return null;
                }
                PurchaseReturnReturn result = new PurchaseReturnReturn(doc);
        ...
        {
            var entity = DC.Set<PurchaseReturn>().Where(x => x.SourceSystemId == sourceSystemId).AsNoTracking().FirstOrDefault();
            if (entity == null)
            {
                return false;
            }
            ...
            {
                if (entity.Status == PurchaseReturnStatusEnum.InWh)
                {
        ...
        public void GetOffDetails(PurchaseReturnReturn doc)
        {
            // 稍后在做，待产生了数据再做
            var invRelations = DC.Set<BaseDocInventoryRelation>().AsNoTracking().Include(x => x.Inventory.WhLocation.WhArea.WareHouse)
                .Where(x => x.BusinessId == doc.ID && x.DocType == DocTypeEnum.PurchaseReturn).ToList();
            ...
            {
                line.SubLines = new List<PurchaseReturnReturnSubLine>();
                invRelations.FindAll(x => x.BusinessLineId == line.ID).ForEach(x =>
                {
                    ...
                    //    return;
                    //}
                    line.SubLines.Add(new PurchaseReturnReturnSubLine
                    {
        ...
        /// </summary>
        /// <param name="doc"></param>
        public void GetSuggestLocs(PurchaseReturnReturn doc)
        {
            foreach (var line in doc.Lines)
            {
                line.SuggestLocs = new List<PurchaseReturnReturnLineSuggestLoc>();
                List<BaseInventory> invs = this.GetItemInventory(line.ItemID, line.WhId);
                if (invs.Count > 0)
                {
                    invs = invs.OrderByDescending(x => x.CreateTime).ToList();  // 采购退货单需按创建时间倒序排列（越新的越可能是退货的物料）
                    foreach (var inv in invs)
                    {
                        line.SuggestLocs.Add(new PurchaseReturnReturnLineSuggestLoc
                        {
        ...
        /// </summary>
        /// <param name="doc"></param>
        public void SaveOffDetails(PurchaseReturnDataPara data, bool autoSplit)
        {
            ...
            {
                PurchaseReturn doc = DC.Set<PurchaseReturn>()             //.AsNoTracking()
                    .Include(x => x.PurchaseReturnLine_PurchaseReturn)
                ...
                if (doc.PurchaseReturnLine_PurchaseReturn.Any(y => y.WareHouseId != whid))
                {
                    MSD.AddModelError("", "单据存储地点不匹配，可尝试重新选择登录存储地点");
                    return;
                }
                if (doc.Status != PurchaseReturnStatusEnum.InWh && doc.Status != PurchaseReturnStatusEnum.PartOff)
                {
                ...
                foreach (var dataLine in data.Lines)
                {
                    var line = doc.PurchaseReturnLine_PurchaseReturn.Find(x => x.ID == dataLine.ID);
                    if (line == null)
                    {
                        MSD.AddModelError("", $"行数据“{dataLine.ID}”不存在");
                        return;
                    }
                    if (line.Status != PurchaseReturnLineStatusEnum.InWh && line.Status != PurchaseReturnLineStatusEnum.PartOff)
                    {
                    ...
                    if (line.ToBeOffQty == 0)
                    {
                        line.Status = PurchaseReturnLineStatusEnum.AllOff;
                    }
                    else if (line.OffQty == 0)
                    {
                        line.Status = PurchaseReturnLineStatusEnum.InWh;
                    }
                    else
                    {
                        line.Status = PurchaseReturnLineStatusEnum.PartOff;
                    }
                }
                ...
                // 设置单据状态
                if (doc.PurchaseReturnLine_PurchaseReturn.All(x => x.Status == PurchaseReturnLineStatusEnum.AllOff))   // 全部行已下架
                {
                    doc.Status = PurchaseReturnStatusEnum.AllOff;
                }
                else if (doc.PurchaseReturnLine_PurchaseReturn.All(x => x.Status == PurchaseReturnLineStatusEnum.InWh))  // 全部行未下架（下架动作不会出现这种情况）
                {
                    doc.Status = PurchaseReturnStatusEnum.InWh;
                }
                else
                {
                    doc.Status = PurchaseReturnStatusEnum.PartOff;
                }
                ...
                {
                    // 创建库存流水
                    if (!this.CreateInvLog(OperationTypeEnum.PurchaseReturnOff, doc.DocNo, sourceInvIds[i], newInventoryList[i].ID, -newInventoryList[i].Qty, newInventoryList[i].Qty))
                    {
                        return;
                    }
                }
                ...
                {
                    // 创建单据库存关联关系
                    if (!this.CreateInvRelation(DocTypeEnum.PurchaseReturn, item.Item2, doc.ID, item.Item1, item.Item3))
                    {
                        return;
                    }
                }
        ...
        public void CancelOff(Guid? docId)
        {
            ...
            try
            {
                using (var trans = DC.BeginTransaction())
                {
                    var doc = DC.Set<PurchaseReturn>().Include(x => x.PurchaseReturnLine_PurchaseReturn).Where(x => x.ID == docId).FirstOrDefault();
                    if (doc == null)
                    {
                        MSD.AddModelError("", "单据不存在");
                        return;
                    }
                    if (doc.Status == PurchaseReturnStatusEnum.InWh)
                    {
                        MSD.AddModelError("", "单据尚未进行下架业务，无法进行取消下架操作");
                        return;
                    }
                    if (doc.Status == PurchaseReturnStatusEnum.PartShipped || doc.Status == PurchaseReturnStatusEnum.AllShipped)
                    {
                        MSD.AddModelError("", "单据已出货，无法进行取消下架操作");
                        return;
                    }
                    // 取消下架
                    foreach (var line in doc.PurchaseReturnLine_PurchaseReturn)
                    {
                        if (line.Status == PurchaseReturnLineStatusEnum.InWh)
                        {
                            continue;
                        }
                        if (line.OffQty <= 0)
                        ...
                        else if (line.Status == PurchaseReturnLineStatusEnum.PartShipped || line.Status == PurchaseReturnLineStatusEnum.AllShipped) // 头部已过滤，不会出现这种情况
                        {
                    ...
                    // 设置单据状态
                    if (doc.PurchaseReturnLine_PurchaseReturn.All(x => x.Status == PurchaseReturnLineStatusEnum.AllOff))   // 全部行已下架（取消下架动作不会出现这种情况）
                    {
                        doc.Status = PurchaseReturnStatusEnum.AllOff;
                    }
                    else if (doc.PurchaseReturnLine_PurchaseReturn.All(x => x.Status == PurchaseReturnLineStatusEnum.InWh))  // 全部行未下架
                    {
                        doc.Status = PurchaseReturnStatusEnum.InWh;
                    }
                    else
                    ...
                    foreach (var relation in invRelations)
                    {
                        ...
                        // 创建库存流水
                        if (!this.CreateInvLog(OperationTypeEnum.PurchaseReturnOffCancel, doc.DocNo, relation.InventoryId, originalInv.ID, -inventory.Qty, inventory.Qty))
                        {
                            return;
                        }
                        // 待发库位的库存信息清零
        ...
        public void Approve(Guid? docId)
        {
            ...
            using (var trans = DC.BeginTransaction())
            {
                var doc = DC.Set<PurchaseReturn>().Include(x => x.PurchaseReturnLine_PurchaseReturn).Include(x => x.Organization).Where(x => x.ID == docId).FirstOrDefault();
                if (doc == null)
                {
                    MSD.AddModelError("", "单据不存在");
                    return;
                ...
                foreach (var line in doc.PurchaseReturnLine_PurchaseReturn)
                {
                    line.Status = PurchaseReturnLineStatusEnum.AllShipped;
                    ...
                    foreach (var relation in matchRelations)
                    {
                        ...
                        // 创建库存流水
                        this.CreateInvLog(OperationTypeEnum.PurchaseReturnApprove, doc.DocNo, inventory.ID, null, -relation.Qty, null);
                ...
                doc.Status = PurchaseReturnStatusEnum.AllShipped;
