...
namespace WalkingTec.Mvvm.Mvc.Admin.ViewModels.FrameworkRoleVMs
{
    public class FrameworkRoleMDVM : BaseCRUDVM<FrameworkRole>
    {
        ...
        public List<FrameworkUser> Users;
        ...
        {
            var userCodes = DC.Set<FrameworkUserRole>().Where(x => x.RoleCode == Entity.RoleCode).Select(x => x.UserCode).ToList();
            Users = DC.Set<FrameworkUser>().Where(x => userCodes.Contains(x.ITCode)).ToList();
        }
    }
}
